...
docker build -t gavsum/multi-server:latest -t gavsum/multi-client:$GIT_SHA -f ./server/Dockerfile ./server
docker build -t gavsum/multi-worker:latest -t gavsum/multi-client:$GIT_SHA -f ./worker/Dockerfile ./worker
